...
# TV Shows directory
$TvShowDir = @()
$TvShowDir = Get-Content "C:\PATH\tvpath.txt"

# Movie directory
$MovieDir = @()
$MovieDir = Get-Content "C:\PATH\moviepath.txt"
...
Write-Host "Finding Movie files over $($MovieSize/1GB)GB and TV Episodes over $($TvShowSize/1GB)GB be patient..." -ForegroundColor Gray

# Find all files larger than $TvShowSize
foreach($d in $TvShowDir){
    $d.Length
    $d = $d.Trim()
    $d.Length
    $LargeTvFiles += Get-ChildItem $d -recurse -Exclude $TVExcludeList | where-object {$_.length -gt $TvShowSize}  | Select-Object FullName,Directory,BaseName,Length
}

# Find all files larger than $MovieSize
foreach($m in $MovieDir){
    $m.Length
    $m = $m.Trim()
    $m.Length
    $LargeMovieFiles = Get-ChildItem $MovieDir -recurse -Exclude $MovieExcludeList | where-object {$_.length -gt $MovieSize}  | Select-Object FullName,Directory,BaseName,Length
}
